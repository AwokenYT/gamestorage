...
export default class Tower {
  ...
  createElement() {
    const content = document.querySelector(
      `.towers-panel .tower-build[data-type="${this.type}"]`
    ...
    this.element.style.top = Utils.toPX(
      (this.row + this.size / 2) * this.boardSize
    );
    this.element.style.left = Utils.toPX(
      (this.col + this.size / 2) * this.boardSize
    );
  ...
  setRangeClasses() {
    if (this.level === 1 || this.getActualRange() !== this.getActualRange(-1)) {
      this.element.classList.remove(
        `tower-range${Math.floor(this.getActualRange(-1))}`
      );
      this.element.classList.add(
        `tower-range${Math.floor(this.getActualRange())}`
      );
    }
  }
