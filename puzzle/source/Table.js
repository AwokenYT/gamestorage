...
// Utils
import List from "../../games/utils/List.js";
import Utils from "../../games/utils/Utils.js";
...
export default class Table {
  ...
  constructor(metrics, instance) {
    ...
    const top =
      (metrics.boardHeight + metrics.boardPadding * 2 - bounds.height) / 2 - 20;
    const left =
      (metrics.boardWidth + metrics.boardPadding * 2 - bounds.width) / 2;
  ...
  findNeighborPieces(other) {
    return this.#pieces.findAll(
      (piece) => piece.id !== other.id && other.isNeighbor(piece)
    );
  }
  ...
  findNeighborSets(other) {
    return this.#sets.findAll(
      (set) => set.id !== other.id && set.isNeighbor(other)
    );
  }
